...
public func collectT<S, U where S: Traversable>(source: S, f: PartialFunction<S.ItemType, U>) -> [U] {
    return reduceT(source, initialValue: Array<U>(), combine: { (total : [U], currentItem : S.ItemType) -> [U] in
...
public func countT<S: Traversable>(source: S, p: (S.ItemType) -> Bool) -> Int {
    return sizeT(filterT(source, includeElement: p))
}
...
- parameter source: Traversable containing the elements to be selected
...
public func dropT<S: Traversable>(source: S, n: Int) -> S {
    if nonEmptyT(source) {
        let result = S.build(foldLeftT(source, initialValue: (sizeT(source), Array<S.ItemType>()), combine: { (result: (index: Int, array: [S.ItemType]), currentItem) -> (Int, [S.ItemType]) in
            if result.index > n {
...
public func dropRightT<S: Traversable>(source: S, n: Int) -> S {
    if nonEmptyT(source) {
        ...
        return S.build(foldRightT(source, initialValue: (0, Array<S.ItemType>()), combine: { (result: (index: Int, array: [S.ItemType]), currentItem) -> (Int, [S.ItemType]) in
...
public func dropWhileT<S: Traversable>(source: S, p: (S.ItemType) -> Bool) -> S {
    if let firstIndex = findIndexOfFirstItemToNotSatisfyPredicate(source, p: p) {
        return dropT(source, n: firstIndex)
    }
    return source
}
...
public func findT<S: Traversable>(source: S, p: (S.ItemType) -> Bool) -> S.ItemType? {
    return reduceT(source, initialValue: nil) { (result, item) -> S.ItemType? in
...
public func filterT<S: Traversable>(source: S, includeElement: (S.ItemType) -> Bool) -> S {
    return S.build(reduceT(source, initialValue: Array<S.ItemType>()) { (filtered, item) -> [S.ItemType] in
...
public func flatMapT<S: Traversable, U>(source: S, transform: (S.ItemType) -> [U]) -> [U] {
    return reduceT(source, initialValue: Array<U>()) { (total, item) -> [U] in
...
public func foldRightT<S: Traversable, U>(source: S, initialValue: U, combine: (U, S.ItemType) -> U) -> U {
    return reduceT(source, initialValue: initialValue, combine: combine)
}
...
public func forAllT<S: Traversable>(source: S, p: (S.ItemType) -> Bool) -> Bool {
    return countT(source, p: p) == sizeT(source)
}
...
public func groupByT<S: Traversable>(source: S, f: Function<S.ItemType, HashableAny>) -> Map<S> {
    return reduceT(source, initialValue: Map<S>()) { (currentMap: Map<S>, currentItem: S.ItemType) -> Map<S> in
...
public func headT<S: Traversable>(source: S) -> S.ItemType? {
    if nonEmptyT(source) {
        return reduceT(takeT(source, n: 1), initialValue: nil, combine: { (item: S.ItemType?, currentItem) -> S.ItemType? in currentItem })
    }
    return nil
}
...
public func lastT<S: Traversable>(source: S) -> S.ItemType? {
    return headT(takeRightT(source, n: 1))
}
...
public func mapConserveT<S: Traversable>(source: S, transform: (S.ItemType) -> S.ItemType) -> S {
    return S.build(reduceT(source, initialValue: Array<S.ItemType>()) { (total, item) -> [S.ItemType] in
        total + [transform(item)]
        })
}
...
public func mkStringT<S: Traversable>(source: S) -> String {
    return mkStringT(source, start: "", separator: "", end: "")
}
...
public func mkStringT<S: Traversable>(source: S, separator: String) -> String {
    return mkStringT(source, start: "", separator: separator, end: "")
}
...
public func mkStringT<S: Traversable>(source: S, start: String, separator: String, end: String) -> String {
    let reduceString = reduceT(source, initialValue: start, combine: { (result: String, item: S.ItemType) -> String in
        return result + "\(item)" + separator
    })
    switch (start, separator, end) {
    case ("", "", ""): return reduceString
    default: return reduceString.substringToIndex(reduceString.startIndex.advancedBy(reduceString.utf16.count - separator.utf16.count)) + end
    }
}
...
public func partitionT<S: Traversable>(source: S, p: (S.ItemType) -> Bool) -> (S, S) {
    return (filterT(source, includeElement: p), filterNotT(source, excludeElement: p))
}
...
public func reverseT<S: Traversable>(source: S) -> S {
    return S.build(reduceT(source, initialValue: Array<S.ItemType>(), combine: { (total, item) -> [S.ItemType] in
        [item] + total
    }))
}
...
public func sizeT<S: Traversable>(source: S) -> Int {
    return reduceT(source, initialValue: 0) { (total, item) -> Int in
        return total + 1
    }
}
...
public func sliceT<S: Traversable>(source: S, from startIndex: Int, until endIndex: Int) -> S {
    ...
    default: return S.build(reduceT(source, initialValue: (0, ArrayT<S.ItemType>())) {
...
- parameter source: Traversable to be sorted
...
public func spanT<S: Traversable>(source: S, p: (S.ItemType) -> Bool) -> (S, S) {
    return (takeWhileT(source, p: p), dropWhileT(source, p: p))
}
...
public func splitAtT<S: Traversable>(source: S, n: Int) -> (S, S) {
    return (takeT(source, n: n), dropT(source, n: n))
}
...
public func takeT<S: Traversable>(source: S, n: Int) -> S {
    return dropRightT(source, n: sizeT(source) - n)
}
...
public func takeRightT<S: Traversable>(source: S, n: Int) -> S {
    return dropT(source, n: sizeT(source) - n)
}
...
public func takeWhileT<S: Traversable>(source: S, p: (S.ItemType) -> Bool) -> S {
    if let firstIndex = findIndexOfFirstItemToNotSatisfyPredicate(source, p: p) {
        return takeT(source, n: firstIndex)
    }
    return source
}
...
public func toArrayT<S: Traversable>(source: S) -> [S.ItemType] {
    return reduceT(source, initialValue: Array<S.ItemType>(), combine: { (total, item) -> [S.ItemType] in
        total + [item]
    })
}
...
public func toListT<S: Traversable>(source: S) -> ListT<S.ItemType> {
    return reduceT(source, initialValue: ListT<S.ItemType>(), combine: { (list : ListT<S.ItemType>, item : S.ItemType) -> ListT<S.ItemType> in
...
private func findIndexOfFirstItemToNotSatisfyPredicate<S: Traversable>(source: S, p: (S.ItemType) -> Bool) -> Int? {
    let result = reduceT(source, initialValue: (0, false)) { (result: (count: Int, didFindItem: Bool), item) -> (Int, Bool) in
